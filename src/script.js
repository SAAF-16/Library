...
myLibrary[0] = new Book("Carillon", 96, "Long story of a short movie." + loremIpsum, 1984, "John Doe", "Drama", 96);
myLibrary[1] = new Book("Jemba", 42, "Jemba jemba jemba is the real jemba jemba." + loremIpsum, 2001, "Jemba!", "Horror", 0);
myLibrary[2] = new Book("The ride", 157, "A long ride taking to never seen spaces." + loremIpsum, 1895, "Carl Carlson", "Drama", 157);
myLibrary[3] = new Book("To the line", 132, "Everything in this book goes to the line." + loremIpsum, 1995, "Krave Denson", "Adventure", 34);
myLibrary[4] = new Book("Unforgiven", 204, "Never forget never forgive." + loremIpsum, 1999, "Zaza Zazovis", "Horror", 204);
...
function update_read_status() {
    return () => {
        myLibrary[active_book].pages == +read_pages.value ?
...
function updateBooks(doc, active) {
    ...
    for (let i = myLibrary.length - 1; i >= 0; i--) {
        //the next if allows me to use the same funcion for different purposes
